...
| T09 | 实现年度相册阅读器 | 接入真实翻页效果，保留首页/上一页/下一页 | 桌面双页、移动单页；按钮行为正确 | 浏览器交互测试；`npm run build` | pending |
| T10 | 实现分享链接 | 生成、复制、撤销年度相册分享链接 | 访客可通过链接观看；撤销后不可访问 | 手动访问测试；`npm run build` | pending |
| T11 | 补齐响应式与上传体验 | 优化移动端、上传进度、加载和错误状态 | 核心流程在桌面和移动端可用 | 浏览器截图审查；`npm run build` | pending |
| T12 | 部署准备 | 添加部署配置、环境变量示例、上线说明 | 项目能按文档部署；敏感配置不入库 | `npm run build`；文档审查 | pending |
