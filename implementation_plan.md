...
| T15 | 持久化上传相册与单页原图阅读 | 上传后写入 MinIO 相册索引；首页/阅读页从 MinIO 恢复；阅读器固定单页；照片使用原图；首页标题改为“流光岁月” | 刷新后可从 MinIO 恢复上传相册；封面首页只显示“谢淑琴”；相册无页码且单页显示；照片页使用原图地址 | `npm run test`；`npm run typecheck`；`npm run build`；API 与浏览器审查 | committed |
| T16 | 优化相册封面质感与翻页尺寸一致性 | 提升相册阅读封面的纸张质感；封面页和照片页共用固定 4:3 画布；照片铺满同尺寸画布 | 封面仍只显示“谢淑琴”；下一页后页面尺寸和封面一致；无旧标题和页码 | `npm run test`；`npm run typecheck`；`npm run build`；浏览器尺寸审查 | committed |
| T17 | 保留完整原图并升级封面封皮 | 照片页改为完整原图等比显示；封面改为深色高级封皮材质；继续只保留姓名行书 | 照片不裁剪且使用原图接口；原图自然尺寸可读；封面只显示“谢淑琴”且更有封皮质感 | `npm run test`；`npm run typecheck`；`npm run build`；浏览器与 API 审查 | committed |
| T18 | 高级中性色相册阅读界面 | 参考高端极简/摄影作品集 UI，移除全黑底色，改为暖灰、鼠尾草绿和浅材质封面 | 页面不再全黑；封面浅色材质且只保留姓名；照片仍完整高清显示 | `npm run test`；`npm run typecheck`；`npm run build`；浏览器审查 | committed |
| T19 | 黑色相框与高级封面首页 | 相册页加入黑色相框、象牙色卡纸和浅色封面内页；照片继续完整高清显示 | 封面和照片页都有黑色相框；照片卡纸为象牙色；封面只显示姓名且更像高级相册首页 | `npm run test`；`npm run typecheck`；`npm run build`；浏览器审查 | committed |
| T20 | 相框外区域高级黑 | 相框外页面背景、分享区、导航和容器改为高级暖黑；保留黑色相框、象牙卡纸和完整原图显示 | 相框外不再浅色；分享和导航为高级黑；相框、卡纸和照片完整性不变 | `npm run test`；`npm run typecheck`；`npm run build`；浏览器审查 | committed |
| T21 | 新增尾页按钮并净化封面米色质感 | 相册阅读器增加尾页按钮；封面内页移除格子纹理，保留纯净米色与轻微光影 | 首页、上一页、下一页、尾页四个按钮可见；尾页跳转到最后一页；封面不再有格子纹理 | `npm run test`；`npm run typecheck`；`npm run build`；源码样式审查 | committed |
| T22 | 修复尾页空照片 | 阅读器只展示真实归档照片，不再把缺失资源的 seed 示例照片追加到尾页；照片列表变化时夹紧当前页索引 | 有上传照片时尾页落到真实归档照片；无真实照片时不出现空照片页；异步刷新后不越界 | `npm run test`；`npm run typecheck`；`npm run build`；源码审查 | committed |

## 当前下一个 Task

计划内 MVP task 已完成；最近补充 T22 尾页空照片修复已完成。
