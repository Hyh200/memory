...
| T03 | 定义本地数据模型 | 2026-06-15 16:22:39 +08:00 | User、Photo、AlbumYear、StyleProfile、CoverAsset 类型；本地 AlbumYearView 数据层；首页接入模型数据 | `npm run typecheck` 通过；`npm run build` 通过；审查确认未引入数据库、上传处理或真实归档算法；`next-env.d.ts` 由 build 自动同步到 `.next/types/routes.d.ts` | 通过 | 待提交 | 审查通过，允许提交 |
