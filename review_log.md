...
| T04 | 实现照片上传入口 | 2026-06-15 16:30:30 +08:00 | `/upload` 页面；多图文件 input；上传队列；格式和大小错误状态；首页入口 | `npm run typecheck` 通过；`npm run build` 通过；浏览器检查 `/upload` 可访问、input accept/multiple 正确、空队列可见、桌面/移动文档层无横向溢出、控制台无 error/warn | 通过 | 待提交 | 审查通过；系统文件选择器注入受限，真实选文件需人工补验 |
