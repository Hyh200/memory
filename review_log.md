...
| T13 | 收敛相册展示 | 2026-06-16 10:44:12 +08:00 | 首页默认仅显示今年相册；封面只保留姓名行书；姓名改为谢淑琴；阅读页照片下方不显示标题/标签/署名，移除结束页文案 | `npm run test` 通过，19 项测试中 18 通过、MinIO 无凭据集成测试跳过；`npm run typecheck` 通过；`npm run build` 通过；浏览器审查 `/` 可见相册卡片 1 个，封面文本仅 `谢淑琴`，旧年份和旧姓名不可见；浏览器审查 `/albums/2026` 无旧姓名、无照片文件名/风格摘要/cover/photo 标签、无横向溢出、控制台无 warn/error | 通过 | 待提交 | 构建仍保留 T05 已记录的 sharp `Couldn't load fs/zlib` 警告但退出码为 0；本 task 未改变上传、MinIO、分享权限逻辑 |
