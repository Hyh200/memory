...
| T12-Flexus | 优化 Flexus L 宝塔上线文档 | 2026-06-16 10:35:21 +08:00 | 根据本地华为云 Flexus L 宝塔面板最佳实践 HTML 优化 `DEPLOYMENT.md`；覆盖安全组、宝塔初始化、Docker 安装、项目启动、宝塔/Nginx 反代 HTTPS、MinIO Console 隧道、备份回滚 | 读取本地 HTML 和资源目录并提取 Flexus L/宝塔正文要点；`git diff --check` 仅有 Windows LF/CRLF 提示；`docker compose -f docker-compose.prod.yml --env-file .env.production config --quiet` 通过；人工审查文档仅包含占位符密钥，明确不开放 `3000`、`9000`、`9001`、`3306`、`9090` 到公网 | 通过 | 待提交 | 官方示例会开放 `3306` 和 `9090`，本项目文档按当前架构改为不建议开放；此补充只改部署文档，不改运行时代码 |
